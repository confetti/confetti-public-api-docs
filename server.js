...
app.get('/example', async (req, res) => {
  ...
  const eventsWithoutOrganisation = await api.get('/events?filter[hasOrganisation]=false')
  ...
  return res.render('example', {
    ...
    eventsWithoutOrganisationResponseBody: JSON.stringify(eventsWithoutOrganisation, null, 2),
...
app.get('/example/organisations/:id', async (req, res) => {
  ...
  const events = await api.get('/events?filter[organisationId]=' + req.params.id)
